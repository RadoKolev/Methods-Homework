...
class NumberCalculations
{
    static void Main()
    {
        Console.WriteLine("Please enter your numbers of type double on a single line, separated by a space: ");
        double[] doubleNumbers = Console.ReadLine().Split(' ').Select(double.Parse).ToArray();

        Console.WriteLine("Please enter your numbers of type decimal on a single line, separated by a space: ");
        decimal[] decimalNumbers = Console.ReadLine().Split(' ').Select(decimal.Parse).ToArray();

        Console.WriteLine("The minimum of the given set is: " + GetMin(doubleNumbers));
